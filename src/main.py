...
repo_version = fetch_version.repo_version()
...
if youtubo_version != repo_version:
    if messagebox.askyesno(title="Update", message=f"Update Available YouTubo - v{youtubo_version}\n"
                                                   f"Kindly download it by clicking 'YES'"):
        webbrowser.open_new_tab(url="https://github.com/abanand132/youtubo/tree/main/application")
    else:
        messagebox.showwarning(title="Important", message="You need to download newer version!!\n"
                                                          "Previous version of YouTubo will not work")
        exit()
